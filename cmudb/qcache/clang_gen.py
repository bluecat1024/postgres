...
def generate_explain(field_map, prefix, obj, pg_type, fields, indent):
    ...
    for field in fields:
        ...
        if pg_type == "Scan" and field.name == "scanrelid":
...
def generate_explain_node_entry(structs):
    # Generic switch-case entry function to route.
    fn_decl = [
        f"static void ExplainEntry(Node *obj, ExplainState *es, EState *estate)"" {",
...
def generate_node_to_name(structs):
    # Generate switch-case entry to get a string identifier from a Node.
    fn_decl = [
        f"static char* NodeToName(Node* obj)"" {",
        construct_with_indent(1, "switch (obj->type) {"),
    ]
...
if __name__ == "__main__":
    ...
    fn_explain_node_entry = generate_explain_node_entry(tags)
    fn_node_to_name = generate_node_to_name(tags)

    with open("qcache.c", "r") as f:
        template = f.read()
